...
typedef struct
   ...
   Byte encrypted_data[Z_BUFSIZE];/* buffer contain compressed data to be writ*/
...
extern int ZEXPORT zipOpenNewFileInZip (file, filename, zipfi, 
   ...
   return zipOpenNewFileInZip2 (file, filename, zipfi, 
                          extrafield_local, size_extrafield_local,
                          extrafield_global, size_extrafield_global,
                          comment, method, level, NULL);
      }

extern int ZEXPORT zipOpenNewFileInZip2(file, filename, zipfi, 
                              extrafield_local, size_extrafield_local,
                              extrafield_global, size_extrafield_global,
                              comment, method, level, szEncryptKey)
   zipFile file;
   const char* filename;
   const zip_fileinfo* zipfi;
   const void* extrafield_local;
   ...
   if (err==Z_OK)
     ...
     *zi->ci.szEncryptKey = '\0';
   if (err==Z_OK && szEncryptKey)
         {
      strcpy (zi->ci.szEncryptKey, szEncryptKey);
      RC4InitSBox (zi->ci.szEncryptKey);
         }
   return err;
   }

static char* BufferData (zipFile file)
      {
   zip_internal* zi;
   zi = (zip_internal*)file;

   if (!*zi->ci.szEncryptKey) 
      return zi->ci.buffered_data;

   return RC4CryptStream (zi->ci.encrypted_data, zi->ci.buffered_data, (uInt)zi->ci.pos_in_buffered_data);
      }
...
//         if (fwrite(zi->ci.buffered_data,(uInt)zi->ci.pos_in_buffered_data,1,zi->filezip)
         if (fwrite(psz,(uInt)zi->ci.pos_in_buffered_data,1,zi->filezip)
                                                         !=1)
            err = ZIP_ERRNO;
         zi->ci.pos_in_buffered_data = 0;
         zi->ci.stream.avail_out = (uInt)Z_BUFSIZE;
